...
function findNextAvailableSlot(
    ...
    for (let dayOffset = 0; dayOffset < SEARCH_DAYS; dayOffset++) {
        ...
        const cursor =
            dayOffset === 0 && searchStart > dayStart ? new Date(searchStart) : new Date(dayStart);
        ...
        while (slot.getTime() + durationMs <= dayEnd.getTime()) {
            ...
            const overlaps = existingEvents.some((e) => slot < e.endTime && slotEnd > e.startTime);
            if (!overlaps) {
                return { start: new Date(slot), end: slotEnd };
            }
...
export async function POST(req: NextRequest) {
    ...
    const existingEvents = await prisma.event.findMany({
        where: { startTime: { lt: searchWindowEnd }, endTime: { gt: now } },
